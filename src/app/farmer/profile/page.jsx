...
const EMPTY_FARMER = {
  ...
  email: "",
  role: "FARMER",
  verification: { isVerified: false, isPhoneVerified: false, verifiedAt: null },
  isActive: true,
...
export default function ProfilePage() {
  ...
  const fetchFarmerProfile = async (showLoader = true) => {
    try {
      ...
      const f = normalizeFarmer(data.farmer || data.data);
      setFarmer(f);
  ...
  const fetchCentres = async () => {
    try {
      ...
      console.error("Centres fetch error:", err);
    }
  };

  useEffect(() => {
    Promise.allSettled([fetchFarmerProfile(true), fetchCentres()]);
  }, []);

  const handleRefresh = async () => {
    await Promise.allSettled([fetchFarmerProfile(false), fetchCentres()]);
  ...
  const getDocStatus = (status) => {
    if (status === "VERIFIED") return { label: "Verified", cls: "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20", Icon: CheckCircle2 };
    if (status === "REJECTED") return { label: "Rejected", cls: "text-rose-600 dark:text-rose-400 bg-rose-500/10 border-rose-500/20", Icon: Clock3 };
    return { label: "Pending", cls: "text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/20", Icon: Clock3 };
  };

  const formatFileSize = (s) => (!s ? "Unknown size" : s < 1024 ? `${s} B` : s < 1048576 ? `${(s / 1024).toFixed(1)} KB` : `${(s / 1048576).toFixed(1)} MB`);
  const formatDate = (d) => (!d || Number.isNaN(new Date(d).getTime()) ? "Not available" : new Date(d).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }));

  const getCentreName = () => {
    const pc = farmer.preferredCentre;
    if (!pc) return "Not selected";
    if (typeof pc === "object") return pc.name || pc.title || pc.code || pc._id || "Selected Centre";
    const found = centres.find((c) => String(c._id) === String(pc));
    return found ? found.name || found.title || found.code || "Selected Centre" : pc;
  };
  ...
  return (
    <div className="h-full w-full overflow-hidden flex flex-col justify-center items-center p-2 sm:p-4 select-none antialiased">
      <div className="w-full max-w-7xl h-full min-h-[92vh] max-h-[94vh] flex flex-col min-h-0 bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl rounded-3xl border border-slate-200/90 dark:border-white/10 shadow-2xl overflow-hidden relative">
        ...
        <div className="flex-1 min-h-0 flex flex-col p-4 sm:p-6 lg:p-7 overflow-hidden">
          <header className="shrink-0 pb-4 border-b border-slate-200/80 dark:border-white/5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div>
              ...
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Farmer Profile</h1>
              <p className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 font-medium">Verified identification, farm registry, and linked MSP documentation.</p>
            </div>
            ...
            <div className="flex items-center gap-2 self-start sm:self-auto">
              {!farmer.onboardingCompleted && (
                <button
                  ...
                  className="h-9 px-3.5 rounded-xl bg-gradient-to-r from-emerald-600 to-lime-600 hover:from-emerald-500 hover:to-lime-500 text-white text-xs font-black flex items-center gap-1.5 shadow-md transition active:scale-95"
                >
              ...
              <button
                ...
                disabled={refreshing}
                className="h-9 px-3.5 rounded-xl border border-slate-200/80 dark:border-white/10 bg-white/90 dark:bg-slate-800/80 text-xs font-bold text-slate-700 dark:text-slate-200 flex items-center gap-2 hover:border-emerald-400 transition shadow-sm disabled:opacity-50"
              >
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pr-1 pt-4 space-y-4 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800 hover:scrollbar-thumb-emerald-500">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-5">
                ...
                <div className="flex-1 text-center sm:text-left">
                  <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-white">{farmer.name || "Farmer"}</h2>
                    {farmer.verification?.isVerified && (
                      <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-400 text-[9px] font-black uppercase">
                  ...
                  <div className="flex flex-wrap justify-center sm:justify-start items-center gap-3 mt-2 text-xs text-slate-500 dark:text-slate-400">
                    <span className="px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 text-[9px] font-black uppercase text-slate-500 dark:text-slate-400">{farmer.role || "FARMER"}</span>
                    <span className="flex items-center gap-1.5"><Phone className="w-3.5 h-3.5 text-emerald-500" />{farmer.mobile ? `+91 ${farmer.mobile}` : "Mobile not set"}</span>
                    <span className="flex items-center gap-1.5"><MapPin className="w-3.5 h-3.5 text-emerald-500" />{farmer.farmLocation?.village || "Village not specified"}{farmer.farmLocation?.district ? `, ${farmer.farmLocation.district}` : ""}</span>
                  </div>
            ...
            <div className="grid lg:grid-cols-2 gap-3.5">
              ...
              <ProfileSection icon={ShieldCheck} title="Verification & Security Audit" desc="Official APMC validation record">
                <VerificationItem label="Identity Verification" value={farmer.verification?.isVerified ? "Admin Approved" : "Pending Verification"} verified={farmer.verification?.isVerified} />
                <VerificationItem label="Mobile Phone Check" value={farmer?.isPhoneVerified ? "SMS Verified" : "Unverified"} verified={farmer?.isPhoneVerified} />
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-white/90 dark:bg-slate-800/60 border border-slate-200/90 dark:border-white/5 shadow-sm">
              <div className="flex items-center gap-2 pb-3 border-b border-slate-100 dark:border-white/5 mb-3">
                <div className="w-8 h-8 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
                  <FileText className="w-4 h-4" />
                </div>
                <div>
                  <h3 className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">Registered Documents ({farmer.documents?.length || 0})</h3>
              ...
              {farmer.documents?.length > 0 ? (
                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-2.5">
                  {farmer.documents.map((doc) => {
                    const { label, cls, Icon: StatusIcon } = getDocStatus(doc.status);
                    return (
                      <div key={doc._id} className="p-3 rounded-xl border border-slate-200/80 dark:border-white/5 bg-slate-50/70 dark:bg-slate-800/40 flex flex-col justify-between">
                        <div>
                          <div className="flex items-start justify-between gap-2">
                            ...
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider ${cls}`}>
                              <StatusIcon className="w-2.5 h-2.5" />{label}
                            </span>
                          </div>
                          <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 mt-0.5">{DOC_TYPES[doc.type] || doc.type}</p>
                        ...
                        >
                          <Eye className="w-3.5 h-3.5 text-emerald-500" />View Document
                        </button>
                      </div>
                    );
                  })}
            ...
            <div className="p-4 sm:p-5 rounded-2xl bg-white/90 dark:bg-slate-800/60 border border-slate-200/90 dark:border-white/5 shadow-sm">
              <div className="flex items-center gap-2 pb-2.5 border-b border-slate-100 dark:border-white/5 mb-3">
                ...
                <h3 className="text-xs font-black uppercase tracking-wider text-slate-900 dark:text-white">Active Dispatch Channels</h3>
              </div>
          ...
          <footer className="shrink-0 pt-3 border-t border-slate-200/80 dark:border-white/10 flex items-center justify-between text-xs text-slate-400">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-semibold text-slate-500 dark:text-slate-400">Official Farmer Registry Data • State APMC Records</span>
            </div>
            <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400">Read-Only Verified Profile</span>
          </footer>
        </div>
      </div>
...
function DocumentViewer({ document: doc, fileSize, formattedDate, onClose }) {
  const isImage = doc.mimeType?.startsWith("image/");
  const isPdf = doc.mimeType === "application/pdf";
  const { label, cls, Icon: StatusIcon } = (() => {
    if (doc.status === "VERIFIED") return { label: "Verified", cls: "text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 border-emerald-500/20", Icon: CheckCircle2 };
    if (doc.status === "REJECTED") return { label: "Rejected", cls: "text-rose-600 dark:text-rose-400 bg-rose-500/10 border-rose-500/20", Icon: Clock3 };
    return { label: "Pending", cls: "text-amber-600 dark:text-amber-400 bg-amber-500/10 border-amber-500/20", Icon: Clock3 };
  })();
